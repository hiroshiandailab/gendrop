// ==========================================================
// MA - Advanced Version
// slow core pulse / thick radiant beams / sacred bloom / dissolve
// ==========================================================
...
function setup() {
  ...
  scene = new SceneController(72.0); // longer cycle
...
function draw() {
  ...
  noStroke();
  fill(PALETTE.bg[0], PALETTE.bg[1], PALETTE.bg[2], 18);
...
class GeoLayer {
  ...
  update(state, breath, bloomStrength, phaseProgress) {
    ...
    for (let i = 0; i <= detail; i++) {
      ...
      // subtle sacred tightening during bloom
  ...
  draw(state, breath, bloomStrength, phaseProgress) {
    ...
    noFill();
    for (let g = 0; g < 4; g++) {
      stroke(c[0], c[1], c[2], alpha * (0.06 - g * 0.01));
    ...
    // double-line structure for temple-like quality
...
class AuraField {
  ...
  draw(state, breath, phaseProgress) {
    ...
    noStroke();
    for (let i = 0; i < 12; i++) {
      let r = map(i, 0, 11, s * 0.07, s * 0.56);
      let a = map(i, 0, 11, 20 + breath * 10, 1);
      fill(PALETTE.sub[0], PALETTE.sub[1], PALETTE.sub[2], a);
      circle(0, 0, r * 2);
    }
    ...
    for (let i = 0; i < 120; i++) {
      ...
      let a = map(noise(i * 0.13, frameCount * 0.0022 + 199), 0, 1, 2, 12);
      fill(255, a);
      circle(x, y, random(0.8, 2.3));
    }
...
function updateParticles(state, breath, phaseProgress, bloomStrength) {
  // during dissolve, sample points from geometry and burst particles outward
...
function drawCore(state, breath, phaseProgress, bloomStrength) {
  ...
  const s = min(width, height);
  const base = s * 0.0105;
  const pulse = base + breath * s * 0.024; // slower and more controlled
  const coreBright = 210 + breath * 30;

  noStroke();

  // layered soft glow
  for (let i = 0; i < 8; i++) {
    let r = pulse + i * (s * 0.006);
    let a = map(i, 0, 7, 34 + breath * 12, 2);
    fill(PALETTE.core[0], PALETTE.core[1], PALETTE.core[2], a);
    circle(0, 0, r * 2);
  }

  fill(PALETTE.core[0], PALETTE.core[1], PALETTE.core[2], coreBright);
  circle(0, 0, pulse * 1.25);

  noFill();
  stroke(PALETTE.core[0], PALETTE.core[1], PALETTE.core[2], 120);
  strokeWeight(1.0 + breath * 0.55);
  circle(0, 0, pulse * 3.1);

  stroke(PALETTE.core[0], PALETTE.core[1], PALETTE.core[2], 58);
  strokeWeight(0.8);
  circle(0, 0, pulse * 5.4);
